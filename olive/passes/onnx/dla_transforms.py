...
def transform_matmul_to_transpose_conv_transpose(model):
    ...
    for node in nodes_to_remove:
        ...
        def check_to_apply_transpose(conv_node):
            ...
            bool_value = False
            if node.input[1] in initializer_names: #### if second input is initializer with suitable conditions, we need to trans-conv-transpose
                for init in model.graph.initializer:
                    if init.name == node.input[1]:
                        shape = list(init.dims)
                        if(len(shape) == 2):
                            # if( shape[1] != 1):          #### changed from != to == (K1) ans removed condition on shape[0](PSO3) and removed condition on shape[1] (PSH)
                            bool_value = True
                        elif(len(shape) == 3):
                            if(shape[0] == 1):
                                bool_value = True
            ...
            else:   #### added this else statement (HT) 
                input = conv_node.input[1]
                if input in tensor_name_dim_map:
                    shape = tensor_name_dim_map[input]
                    if len(shape) == 2:
                        # if shape[1] != 1:  # The 2D tesnor (MxN, width and height) will later adding leading dimensions to 4D with 1x1xMxN #### changed from != to == (K1) ans removed condition on shape[0](PSO3) and removed condition on shape[1] (PSH)
                        bool_value = True
                    ...
                    if len(shape) == 4 and shape[2] != 1 and shape[3] != 1: # In some cases, C != 1 but it still needs the transpose
                        bool_value = True
            if(not(bool_value)):return bool_value
            bool_value = False
            input = conv_node.input[0]
            if input in tensor_name_dim_map:
                shape = tensor_name_dim_map[input]
                if len(shape) == 2 and shape[1] != 1:  # The 2D tesnor (MxN, width and height) will later adding leading dimensions to 4D with 1x1xMxN #### changed from != to == (K1) ans removed condition on shape[0](PSO3)
                    bool_value = True
                if len(shape) == 3 and shape[0] == 1: # C == 1
                    bool_value = True
                if len(shape) == 4 and shape[1] == 1: # C == 1  
        ...
        if need_to_apply_first_transpose:      #### START OF block of items newly added
            ...
            conv_inputs = [conv_input0, conv_input1]    #### END OF block of items newly added
            # transpose_before_node = helper.make_node(
            #     'Transpose',
            #     inputs=[node.input[0]],
            #     outputs=[f"{matmul_node_name}_transpose_before_output_{cnt}"],
            #     name=f"{matmul_node_name}_transpose_before_{cnt}",
            #     perm=[0,3,2,1]
            # )
            # nodes_to_add.append(transpose_before_node)
            # conv_inputs = [f"{matmul_node_name}_transpose_before_output_{cnt}", node.input[1]]
        else:
        ...
        if(need_to_apply_transpose):
            ...
            node_to_add = helper.make_node(    #### we dont want mul/gemm to get replaced by conv so added this else statement (PSW)
                node.op_type,
                inputs=conv_inputs,
                outputs=node.output,
                name=f"{matmul_node_name}_new"
            )
        ...
        cnt += 1
    #### Was giving error when removing initializers so checking if it exists first ####
...
def transform_remove_intermediary_squeeze_and_unsqueeze(model):
    ...
    for node in graph.node:
        ...
        if node.op_type == 'Squeeze' and input_shape is not None and len(input_shape) == 5:  ############### added for PSP1 as squeeze was removing axis 2 and not 0...very model specific approach used here, fixing the size and axis in the condition, but this can be improved
        ...
        if (node.op_type == 'Squeeze' or (node.op_type == "Unsqueeze" and len(input_shape)==4)) and node.input[0] not in input_names and node.output[0] not in output_names:
        ...
        if node.op_type == 'Unsqueeze' and node.input[0] not in input_names and node.output[0] not in output_names:
            ...
            if (len(input_shape) == 2 and len(out_shape) == 4) or \
               (len(input_shape) == 3 and len(out_shape) == 4) or \
...
def transform_qdq_to_clip(model):
    ...
    for q_output, qualin_node in qualin_name_node_map.items():
        if q_output in deqlin_name_node_map and (node_datatype_map[q_output] in ["uint16", "int16"]):
            ...
            clip_min_init = numpy_helper.from_array(np.array(clip_min, dtype=np.float32), deqlin_node.name + '_clip_min')
            clip_max_init = numpy_helper.from_array(np.array(clip_max, dtype=np.float32), deqlin_node.name + '_clip_max')
    ...
    for q_output, q_node in q_output_to_q_node_map.items():
        if q_output in dq_input_to_dq_node_map and (node_datatype_map[q_output] in ["uint16", "int16"]):
            ...
            if keep_clip_after_inputs and q_node.input[0] in [graph_input.name for graph_input in model.graph.input]:
                ...
                clip_min_init = numpy_helper.from_array(np.array(clip_min, dtype=np.float32), dq_node.name + '_clip_min')
                clip_max_init = numpy_helper.from_array(np.array(clip_max, dtype=np.float32), dq_node.name + '_clip_max')
                model.graph.initializer.extend([clip_min_init, clip_max_init])
                clip_node = helper.make_node(
                    'Clip',
                    ...
                    if node.op_type == "Unsqueeze":
                        ...
                        if existing_unsqueeze_axes is not None:
                            ...
                            if(existing_unsqueeze_axes.tolist()[0] == -1):
                                unsqueeze_arr = np.array([-1]+[0], dtype=np.int64) #### Added while dealing with PSH but also in general, this is the correct way to do it
                            else:
                                unsqueeze_arr = np.array([existing_unsqueeze_axes.tolist()[0]+1]+[0], dtype=np.int64) #### Added while dealing with PSW but also in general, this is the correct way to do it
                        ...
                        unsqueeze_arr = np.array([0,1], dtype=np.int64) if dims == 2 else np.array([0], dtype=np.int64)#### changed from 0,-1 to 0,1 (PSW) , if things break, change it back and 0 to 1 (HT)
            ...
            for node in graph.node:
                for i, node_output_name in enumerate(node.output):
                    if node_output_name == graph_output.name:
                        if(get_shape_from_graph(graph,node_output_name) != get_shape_from_graph(graph,graph_output.name)):
            ...
            for node in graph.node:
                ...
                for i, node_input_name in enumerate(node.input):
                    if node_input_name == graph_output.name:
                        if(get_shape_from_graph(graph,node_input_name) != get_shape_from_graph(graph,graph_output.name)):
        ...
        if output_dim < 4 and output_dim > 1:
            for node in graph.node:
                if node.op_type == 'Squeeze' and (node.output[0] == graph_output.name or is_squeeze_clip_output_pattern(node, model)):
                    ...
                    if output_dim == 2:
                        squeeze_axes_arr = np.array([0, 1], dtype=np.int64) #### it was 3 here before, made it 1 (PSI)
                    if output_dim == 3:           #### this case was missing (PSU0)
            ...
            if not update_existing_squeeze:
                ...
                if output_dim == 2:
                    ...
                    squeeze_axes_arr = np.array([0], dtype=np.int64)#### it was 2 here before (PSI)
                squeeze_axes = numpy_helper.from_array(squeeze_axes_arr, squeeze_axes_name)
                graph.initializer.append(squeeze_axes)
...
def transform_reducemin_keepdims_GT(model):
    graph = model.graph
    for node in graph.node:
        if node.op_type == 'ReduceMin' and not 'transformed' in node.name:
            for attr in node.attribute:
...
def transform_standalone_reducesum_reducemean(model):  #### To make it suitable for running F2, i have added reducemean and to make it suitable for GT, i have added reducemin as the core tranformation is exactly same
    ...
    for node in graph.node:
        ...
        if (node.op_type == 'ReduceSum' or node.op_type == 'ReduceMean' or node.op_type == 'ReduceMax') and not 'transformed' in node.name: ######## added ReduceMax (PSA)
            # print(node.name)
            # print(node)
            # set keepdims = 1
            ...
            for initializer in graph.initializer:
                if initializer.name == axes_initializer:
                    ...
                    # if True:     #### Added this (PSO3) , but clearly this condtition is not entrirely correct and need to change it a bit #### it had current_axes.size == 1 and current_axes[0] == -1 now it has True...check if it effects (PSO3)
                    reducesum_axes_name = node.name + '_axes'
...
def transform_non4d_initializers(model):
    ...
    skip_init_names = []
    just_append_to_beginning = []
    unary_dim_at_front_init_names = []
    avoid_expanding_4D_init_names = []
    for node in model.graph.node:
        if node.op_type in ['Div', 'Sub', 'Mul','Add']: #### Added Add (PSD6)
            for input in node.input:
                need_to_expand_4D_init_names.append(input)
        if node.op_type in ['MatMul']:
            for input in node.input:
                skip_init_names.append(input)
        if node.op_type in ['Gemm','Where','Gather','Split']: #### Added Where (PSO3)
            for input in node.input:
                unary_dim_at_front_init_names.append(input)
        if node.op_type in ['Unsqueeze']: #### Avoid expanding 4D for Unsqueeze inputs (HT)
            for input in node.input:
                avoid_expanding_4D_init_names.append(input)

    initializers_to_add = []
    initializer_to_remove = []
    for initializer in model.graph.initializer:
        if len(initializer.dims) == 1 and initializer.name in need_to_expand_4D_init_names and initializer.name not in avoid_expanding_4D_init_names:
            # 1D: [K] -> [1x1x1xK]
            initializer.dims.insert(0, 1)
            initializer.dims.insert(0, 1)
            initializer.dims.insert(0, 1)
        elif len(initializer.dims) == 2 and initializer.name in need_to_expand_4D_init_names and initializer.name not in avoid_expanding_4D_init_names: #### Added this elif while dealing with PSD6
            initializer.dims.insert(0, 1)
            initializer.dims.insert(0, 1)
        elif len(initializer.dims) == 2 and initializer.name in unary_dim_at_front_init_names and initializer.name not in avoid_expanding_4D_init_names:
            # 2D: [K, C] -> [1, 1, K, C]
            new_dims = [1, 1] + list(initializer.dims)
            initializer.dims[:] = new_dims
        elif len(initializer.dims) == 2 and initializer.name not in skip_init_names and initializer.name not in avoid_expanding_4D_init_names:
            # 2D [CxK] -> [KxCx1x1]
            c,k = initializer.dims[0], initializer.dims[1]
            init_arr = numpy_helper.to_array(initializer)
...
def transform_non4d_reshape(model):
    ...
    for node in model.graph.node:
        if node.op_type =='Reshape':
            ...
            for init in model.graph.initializer:
                if init.name == node.input[1]:
                    if init.dims[0] == 2:        #### 2d case not handled by them (C2)
                        # eg [-1, 512] -> [1, 1, -1, 512]
                    ...
                    if init.dims[0] == 3:
                        ...
                        model.graph.initializer.append(new_init)
                        cnt += 1
                    if(init.dims[0] == 5): ######## added for newer version of PSP
                        old_shape = numpy_helper.to_array(init)
                        if(old_shape[0]==1):
                            init.dims[0] = 4
...
def transform_reshape_clip_reducesum(model): ######## added for newer version of PSF,PSK,PSL
    graph = model.graph
    nodes_to_remove = []
    nodes_to_add = []
    counter = 0
    for node in list(graph.node):
        if node.op_type == "Reshape":
            ...
            slice_0_starts = helper.make_tensor(name=f"slice_0_starts_{counter}", data_type=TensorProto.INT64, dims=[1], vals=[0])
            slice_0_ends = helper.make_tensor(name=f"slice_0_ends_{counter}", data_type=TensorProto.INT64, dims=[1], vals=[4])
            slice_1_starts = helper.make_tensor(name=f"slice_1_starts_{counter}", data_type=TensorProto.INT64, dims=[1], vals=[4])
            slice_1_ends = helper.make_tensor(name=f"slice_1_ends_{counter}", data_type=TensorProto.INT64, dims=[1], vals=[8])
            slice_axes = helper.make_tensor(name=f"slice_axes_{counter}", data_type=TensorProto.INT64, dims=[1], vals=[3])
            ...
            # Slice 0 branch
            slice0 = helper.make_node("Slice", [input_tensor, f"slice_0_starts_{counter}", f"slice_0_ends_{counter}", f"slice_axes_{counter}"], [slice0_out])
            ...
            # Slice 1 branch
            slice1 = helper.make_node("Slice", [input_tensor, f"slice_1_starts_{counter}", f"slice_1_ends_{counter}", f"slice_axes_{counter}"], [slice1_out])
...
def transform_split(model): ####### added for newer version of PSA
    graph = model.graph
    for node in graph.node:
    ...
    for node in graph.node:
        if node.op_type == 'Concat':
            ...
            if len(input_shapes) == len(node.input):
                dims_set = set(len(s) for s in input_shapes)
                if len(dims_set) == 1:
                    dims = dims_set.pop()
                    for attr in node.attribute:
                        if attr.name == 'axis' and attr.i != -1: ####### added the second condition for newer version of PSA
                            if dims == 3:
                                attr.i += 1
                                cnt_3d += 1
                            elif dims == 2:
        ...
        if node.op_type == "InstanceNormalization":
            ...
            if(len(input_shape) == 3):
                ...
                if C != scale_len:
                    ...
                    reshape_shape_init = numpy_helper.from_array(np.array(new_shape, dtype=np.int64), reshape_shape_name)
                    graph.initializer.append(reshape_shape_init)
...
def transform_decompose_lstm(model):
    ...
    for node in list(graph.node):
        if node.op_type == "LSTM":
            ...
            x_t_slice = helper.make_node("Slice", [X, zero_initializer_name, one_initializer_name, ax_name, ste_name], [x_t_name], name=f"slice_x_t_{lstm_counter}")
            ...
            split_4h = helper.make_node(
                ...
                [],
                [split_4h_name],
                value=helper.make_tensor(f"split_4h_val_{lstm_counter}", TensorProto.INT64, [4], [hidden_size, hidden_size, hidden_size, hidden_size]),
            ...
            split_8h = helper.make_node(
                ...
                [],
                [split_8h_name],
                value=helper.make_tensor(f"split_8h_val_{lstm_counter}", TensorProto.INT64, [2], [4*hidden_size, 4*hidden_size]),
                name=f"const_split_8h_{lstm_counter}"
            )
            split_W = helper.make_node("Split", [W, split_4h_name], [f"W_i_{lstm_counter}", f"W_o_{lstm_counter}", f"W_f_{lstm_counter}", f"W_c_{lstm_counter}"], axis=2, name=f"split_W_{lstm_counter}")
            split_R = helper.make_node("Split", [R, split_4h_name], [f"R_i_{lstm_counter}", f"R_o_{lstm_counter}", f"R_f_{lstm_counter}", f"R_c_{lstm_counter}"], axis=2, name=f"split_R_{lstm_counter}")
            split_B = helper.make_node("Split", [B, split_8h_name], [f"B_W_{lstm_counter}", f"B_R_{lstm_counter}"], axis=3, name=f"split_B_{lstm_counter}")
            split_B_W = helper.make_node("Split", [f"B_W_{lstm_counter}", split_4h_name], [f"b_Wi_{lstm_counter}", f"b_Wo_{lstm_counter}", f"b_Wf_{lstm_counter}", f"b_Wc_{lstm_counter}"], axis=3, name=f"split_B_W_{lstm_counter}")
            split_B_R = helper.make_node("Split", [f"B_R_{lstm_counter}", split_4h_name], [f"b_Ri_{lstm_counter}", f"b_Ro_{lstm_counter}", f"b_Rf_{lstm_counter}", f"b_Rc_{lstm_counter}"], axis=3, name=f"split_B_R_{lstm_counter}")

            # Add Transpose nodes for weights before MatMul
            transpose_Wi = helper.make_node(
                'Transpose', [f"W_i_{lstm_counter}"], [f"W_i_T_{lstm_counter}"], name=f"transpose_Wi_{lstm_counter}", perm=[0,2,1])
            transpose_Wf = helper.make_node(
                'Transpose', [f"W_f_{lstm_counter}"], [f"W_f_T_{lstm_counter}"], name=f"transpose_Wf_{lstm_counter}", perm=[0,2,1])
            transpose_Wo = helper.make_node(
                'Transpose', [f"W_o_{lstm_counter}"], [f"W_o_T_{lstm_counter}"], name=f"transpose_Wo_{lstm_counter}", perm=[0,2,1])
            transpose_Wc = helper.make_node(
                'Transpose', [f"W_c_{lstm_counter}"], [f"W_c_T_{lstm_counter}"], name=f"transpose_Wc_{lstm_counter}", perm=[0,2,1])
            transpose_Ri = helper.make_node(
                'Transpose', [f"R_i_{lstm_counter}"], [f"R_i_T_{lstm_counter}"], name=f"transpose_Ri_{lstm_counter}", perm=[0,2,1])
            transpose_Rf = helper.make_node(
                'Transpose', [f"R_f_{lstm_counter}"], [f"R_f_T_{lstm_counter}"], name=f"transpose_Rf_{lstm_counter}", perm=[0,2,1])
            transpose_Ro = helper.make_node(
                'Transpose', [f"R_o_{lstm_counter}"], [f"R_o_T_{lstm_counter}"], name=f"transpose_Ro_{lstm_counter}", perm=[0,2,1])
            transpose_Rc = helper.make_node(
                'Transpose', [f"R_c_{lstm_counter}"], [f"R_c_T_{lstm_counter}"], name=f"transpose_Rc_{lstm_counter}", perm=[0,2,1])

            # Gates: i, f, o, g
            # Input gate
            xW_i = helper.make_node("MatMul", [x_t_name, f"W_i_T_{lstm_counter}"], [f"xW_i_{lstm_counter}"], name=f"matmul_xW_i_{lstm_counter}")
            hR_i = helper.make_node("MatMul", [initial_h, f"R_i_T_{lstm_counter}"], [f"hR_i_{lstm_counter}"], name=f"matmul_hR_i_{lstm_counter}")
            b_i = helper.make_node("Add", [f"b_Wi_{lstm_counter}", f"b_Ri_{lstm_counter}"], [f"b_i_{lstm_counter}"], name=f"add_b_i_{lstm_counter}")
            i_t_pre = helper.make_node("Add", [f"xW_i_{lstm_counter}", f"hR_i_{lstm_counter}"], [f"i_t_pre1_{lstm_counter}"], name=f"add_i_t_pre1_{lstm_counter}")
            i_t_pre2 = helper.make_node("Add", [f"i_t_pre1_{lstm_counter}", f"b_i_{lstm_counter}"], [f"i_t_pre2_{lstm_counter}"], name=f"add_i_t_pre2_{lstm_counter}")
            i_t = helper.make_node("Sigmoid", [f"i_t_pre2_{lstm_counter}"], [f"i_t_{lstm_counter}"], name=f"sigmoid_i_t_{lstm_counter}")
            # Forget gate
            xW_f = helper.make_node("MatMul", [x_t_name, f"W_f_T_{lstm_counter}"], [f"xW_f_{lstm_counter}"], name=f"matmul_xW_f_{lstm_counter}")
            hR_f = helper.make_node("MatMul", [initial_h, f"R_f_T_{lstm_counter}"], [f"hR_f_{lstm_counter}"], name=f"matmul_hR_f_{lstm_counter}")
            b_f = helper.make_node("Add", [f"b_Wf_{lstm_counter}", f"b_Rf_{lstm_counter}"], [f"b_f_{lstm_counter}"], name=f"add_b_f_{lstm_counter}")
            f_t_pre = helper.make_node("Add", [f"xW_f_{lstm_counter}", f"hR_f_{lstm_counter}"], [f"f_t_pre1_{lstm_counter}"], name=f"add_f_t_pre1_{lstm_counter}")
            f_t_pre2 = helper.make_node("Add", [f"f_t_pre1_{lstm_counter}", f"b_f_{lstm_counter}"], [f"f_t_pre2_{lstm_counter}"], name=f"add_f_t_pre2_{lstm_counter}")
            f_t = helper.make_node("Sigmoid", [f"f_t_pre2_{lstm_counter}"], [f"f_t_{lstm_counter}"], name=f"sigmoid_f_t_{lstm_counter}")
            # Output gate
            xW_o = helper.make_node("MatMul", [x_t_name, f"W_o_T_{lstm_counter}"], [f"xW_o_{lstm_counter}"], name=f"matmul_xW_o_{lstm_counter}")
            hR_o = helper.make_node("MatMul", [initial_h, f"R_o_T_{lstm_counter}"], [f"hR_o_{lstm_counter}"], name=f"matmul_hR_o_{lstm_counter}")
            b_o = helper.make_node("Add", [f"b_Wo_{lstm_counter}", f"b_Ro_{lstm_counter}"], [f"b_o_{lstm_counter}"], name=f"add_b_o_{lstm_counter}")
            o_t_pre = helper.make_node("Add", [f"xW_o_{lstm_counter}", f"hR_o_{lstm_counter}"], [f"o_t_pre1_{lstm_counter}"], name=f"add_o_t_pre1_{lstm_counter}")
            o_t_pre2 = helper.make_node("Add", [f"o_t_pre1_{lstm_counter}", f"b_o_{lstm_counter}"], [f"o_t_pre2_{lstm_counter}"], name=f"add_o_t_pre2_{lstm_counter}")
            o_t = helper.make_node("Sigmoid", [f"o_t_pre2_{lstm_counter}"], [f"o_t_{lstm_counter}"], name=f"sigmoid_o_t_{lstm_counter}")
            # Cell gate
            xW_c = helper.make_node("MatMul", [x_t_name, f"W_c_T_{lstm_counter}"], [f"xW_c_{lstm_counter}"], name=f"matmul_xW_c_{lstm_counter}")
            hR_c = helper.make_node("MatMul", [initial_h, f"R_c_T_{lstm_counter}"], [f"hR_c_{lstm_counter}"], name=f"matmul_hR_c_{lstm_counter}")
            b_c = helper.make_node("Add", [f"b_Wc_{lstm_counter}", f"b_Rc_{lstm_counter}"], [f"b_c_{lstm_counter}"], name=f"add_b_c_{lstm_counter}")
            g_t_pre = helper.make_node("Add", [f"xW_c_{lstm_counter}", f"hR_c_{lstm_counter}"], [f"g_t_pre1_{lstm_counter}"], name=f"add_g_t_pre1_{lstm_counter}")
            g_t_pre2 = helper.make_node("Add", [f"g_t_pre1_{lstm_counter}", f"b_c_{lstm_counter}"], [f"g_t_pre2_{lstm_counter}"], name=f"add_g_t_pre2_{lstm_counter}")
            g_t = helper.make_node("Tanh", [f"g_t_pre2_{lstm_counter}"], [f"g_t_{lstm_counter}"], name=f"tanh_g_t_{lstm_counter}")

            f_c_prev = helper.make_node("Mul", [f"f_t_{lstm_counter}", initial_c], [f"f_c_prev_{lstm_counter}"], name=f"mul_f_c_prev_{lstm_counter}")
            i_g = helper.make_node("Mul", [f"i_t_{lstm_counter}", f"g_t_{lstm_counter}"], [f"i_g_{lstm_counter}"], name=f"mul_i_g_{lstm_counter}")
            c_t = helper.make_node("Add", [f"f_c_prev_{lstm_counter}", f"i_g_{lstm_counter}"], [node.output[2]], name=f"add_c_t_{lstm_counter}")
            tanh_c_t = helper.make_node("Tanh", [node.output[2]], [f"tanh_c_t_{lstm_counter}"], name=f"tanh_c_t_{lstm_counter}")
            h_t = helper.make_node("Mul", [f"o_t_{lstm_counter}", f"tanh_c_t_{lstm_counter}"], [node.output[0]], name=f"mul_h_t_{lstm_counter}")
            indentity_i = helper.make_node("Identity", [node.output[0]], [node.output[1]], name=f"identity_i_{lstm_counter}")
            ...
            nodes_to_remove.append(node)
            nodes_to_add.extend([
                x_t_slice, split_4h, split_8h, split_W, split_R, split_B, split_B_W, split_B_R,
                transpose_Wi, transpose_Wf, transpose_Wo, transpose_Wc,
                transpose_Ri, transpose_Rf, transpose_Ro, transpose_Rc,
                xW_i, hR_i, b_i, i_t_pre, i_t_pre2, i_t,
                xW_f, hR_f, b_f, f_t_pre, f_t_pre2, f_t,
                xW_o, hR_o, b_o, o_t_pre, o_t_pre2, o_t,
                xW_c, hR_c, b_c, g_t_pre, g_t_pre2, g_t,
                f_c_prev, i_g, c_t, tanh_c_t, h_t,indentity_i
            ])
            lstm_counter += 1
...
def transform_reshape_clip_reducesum_manual(model): ######## added for newer version of PSF,PSK,PSL
    import onnx
    from onnx import helper, TensorProto
    graph = model.graph
    nodes_to_remove = []
...
def transform_reshape_transpose_reshape(model):
    ...
    for node in list(graph.node):
        if node.op_type == "Reshape":
            ...
            transpose_out = transpose_node.output[0]
            # Find second Reshape
            reshape2_nodes = [n for n in graph.node if n.input and n.input[0] == transpose_out and n.op_type == "Reshape"]
            if not reshape2_nodes:
                continue
            reshape2_node = reshape2_nodes[0]
            ...
            for i in range(6):
                # Slice
                slice_out = f"slice_{i}_out_{counter}"
                slice_node = helper.make_node("Slice", [input_tensor, f"slice_{i}_starts_{counter}", f"slice_{i}_ends_{counter}", f"slice_axes_{counter}"], [slice_out])
                # Reshape to 4x4x115x199
                reshape_4d_out = f"reshape_4d_{i}_out_{counter}"
                reshape_4d_node = helper.make_node("Reshape", [slice_out, f"reshape_4d_shape_{counter}"], [reshape_4d_out])
                # Transpose to 115x4x199x4
                transpose_out = f"transpose_{i}_out_{counter}"
                transpose_node = helper.make_node("Transpose", [reshape_4d_out], [transpose_out], perm=[2,0,3,1])
                # Reshape to 1x1x460x796
                reshape_1d_out = f"reshape_1d_{i}_out_{counter}"
                reshape_1d_node = helper.make_node("Reshape", [transpose_out, f"reshape_1d_shape_{counter}"], [reshape_1d_out])
                slice_outputs.append(reshape_1d_out)
                nodes_to_add.extend([slice_node, reshape_4d_node, transpose_node, reshape_1d_node])
